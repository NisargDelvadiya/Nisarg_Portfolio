 ...
 * Displays blog publications and external articles with a realistic browser address bar.
 */
  ...
  return (
    <div
      className={`flex flex-col bg-white dark:bg-[#1e1e22] text-gray-800 dark:text-white rounded-xl shadow-2xl overflow-hidden border border-black/10 dark:border-white/10 select-none transition-colors duration-200 ${
        isMaximized ? 'w-full h-[calc(100vh-140px)]' : 'w-[680px] max-w-[92vw]'
      }`}
    >
      ...
      <div id="window-header" className="bg-gray-100 dark:bg-[#26262b] border-b border-gray-200 dark:border-white/10 px-4 py-2.5 flex items-center justify-between flex-shrink-0">
        ...
        <div
          className="search flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-[#16161a] border border-gray-300 dark:border-white/10 w-3/4 max-w-md mx-auto"
        ...
        >
          ...
          <input
            ...
            className="w-full text-xs text-gray-700 dark:text-gray-200 bg-transparent outline-none cursor-text truncate select-text"
      ...
      {/* Browser Body */}
      <main
        className={`blog flex-1 overflow-y-auto p-4 sm:p-8 bg-white dark:bg-[#18181c] ${
          isMaximized ? 'max-h-[calc(100vh-180px)]' : 'max-h-[70vh]'
        }`}
      >
        <h2 className="text-lg sm:text-xl font-bold text-pink-600 dark:text-pink-500 mb-4 sm:mb-6">
          Blog Posts & Articles
        </h2>
        <div className="space-y-4 sm:space-y-6">
          {blogPosts.map((post) => (
            <article
              key={post.id}
              className="blog-post flex flex-col sm:flex-row gap-3 sm:gap-5 p-3 sm:p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors border border-transparent hover:border-black/5 dark:hover:border-white/5"
            >
              ...
              <div className="content space-y-1.5 sm:space-y-2 min-w-0">
